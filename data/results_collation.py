                ...
                for extra in ("", "-CD"):
                    ...
                    if os.path.exists(full_path):
                        with open(full_path, 'r', encoding="utf-8") as f:
                            data = json.load(f)
                        scores = data.get("performance_metrics", {})
                    ...
                    if args.more_metrics:
                        df.loc[f"{row_idx}_f1", col] = f1
                        df.loc[f"{row_idx}_precision", col] = precision
                        df.loc[f"{row_idx}_recall", col] = recall
                    else:
                        df.loc[row_idx, col] = f1
...
def create_dataframe_llm(args):
    ...
    for key in data:
        ...
        if args.more_metrics:
            df.loc[f"{index}_f1", type_model] = f1
            df.loc[f"{index}_precision", type_model] = precision
            df.loc[f"{index}_recall", type_model] = recall
        else:
            df.loc[index, type_model] = f1 
...
def main():
    ...
    parser.add_argument("--more_metrics", action='store_true', help="Also include the precision and recall in the score")

    args = parser.parse_args()

    output_dir = os.path.join(args.path, "combined_results") if not args.more_metrics else os.path.join(args.path, "combined_results/extended")
