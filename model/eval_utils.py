"""
Code is inspired by https://github.com/jpeper/GEN_SCL_NAT.git and https://github.com/ZubinGou/multi-view-prompting.git
"""
...
# Inspired by: https://github.com/CasHensen/WSG-ASQP.git 
def compute_f1_scores(pred_pt, gold_pt, silent=True, indiv=False):
    """
    ...
    for i in range(len(pred_pt)):
        if not indiv:
            ...
            n_gold += len(list(gold_pt[i]))
            n_pred += len(list(pred_pt[i]))

            label_instance = gold_pt[i].copy()
            for t in list(pred_pt[i]):
                if t in label_instance:
                    n_tp += 1
                    label_instance.remove(t)
